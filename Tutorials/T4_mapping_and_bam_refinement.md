...
```
Name the extensions of the files (e.g. ‘.txt’, ‘.sam’) that have been created for indices of
- samtools:    
- bwa:
- picard:
...
```
> - `-M` *Mark shorter split hits as secondary (for Picard compatibility). See [here](https://www.biostars.org/p/97323/). With option -M it is flagged as a duplicate flag=256 ( not primary alignment ): will be ignored by most 'old' tools.
...
Can you guess the extension of this file? Check it in your folder… (use unix ls
and options)
